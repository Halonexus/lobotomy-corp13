...
/datum/secrets_menu/ui_act(action, params)
	...
	switch(action)
		...
		if("clear_virus")
			...
			if(choice == "Yes")
				...
				for(var/thing in SSdisease.active_diseases)
					...
					D.cure(0)
		if("list_bombers")
			var/dat = "<!DOCTYPE html><B>Bombing List</B><HR>"
			for(var/l in GLOB.bombers)
				dat += text("[l]<BR>")
			holder << browse(dat, "window=bombers")

		if("list_signalers")
			var/dat = "<!DOCTYPE html><B>Showing last [length(GLOB.lastsignalers)] signalers.</B><HR>"
			for(var/sig in GLOB.lastsignalers)
				dat += "[sig]<BR>"
			holder << browse(dat, "window=lastsignalers;size=800x500")
		if("list_lawchanges")
			var/dat = "<!DOCTYPE html><B>Showing last [length(GLOB.lawchanges)] law changes.</B><HR>"
		...
		if("manifest")
			var/dat = "<!DOCTYPE html><B>Showing Crew Manifest.</B><HR>"
		...
		if("dna")
			var/dat = "<!DOCTYPE html><B>Showing DNA from blood.</B><HR>"
		...
		if("fingerprints")
			var/dat = "<!DOCTYPE html><B>Showing Fingerprints.</B><HR>"
